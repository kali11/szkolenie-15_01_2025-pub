...
resource "google_cloud_run_v2_service" "apps" {
  for_each = local.app_configs_safe
  ...
  template {
    ...
    containers {
      ...
      dynamic "env" {
        # Iterate over the env_vars map
        for_each = each.value.env_vars
        content {
          name  = env.key   # Key from the map
          value = env.value # Value from the map
        }
      }

      # DYNAMIC BLOCK: Only creates env blocks for secrets if secret_env_vars map is not empty
      dynamic "env" {
